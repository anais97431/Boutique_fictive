...
function img_load($id_product)
{
        ...
        foreach ($_FILES['photos']['name'] as $key => $val) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["photos"]["tmp_name"][$key], $targetFilePath)) {
                    // Image db insert sql
                    $insertValuesSQL .= "('" . $fileName . "', NOW(), $id_product),";
                } else {
    ...
    $sth->execute();
        ...
        if (!empty($insertValuesSQL)) {
            ...
            $insert = $connection->query("INSERT INTO pictures (title_picture, date_img, id_product) VALUES $insertValuesSQL");
            if ($insert) {
